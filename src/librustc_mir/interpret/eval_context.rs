...
impl<'a, 'mir, 'tcx: 'mir, M: Machine<'mir, 'tcx>> EvalContext<'a, 'mir, 'tcx, M> {
    ...
    pub fn validate_ptr_target(
        ...
        match layout.fields {
            ...
            layout::FieldPlacement::Union(n) => {
                // one of the fields needs to match, doesn't matter which, we leave that to
                // the user. They are in unsafe territory anyway
                for i in 0..n {
                    let field_layout = layout.field(self, i)?;
                    let mut path = path.clone();
                    self.write_field_name(&mut path, layout.ty, i, variant).unwrap();
                    if self.validate_ptr_target(ptr, ptr_align, field_layout, path, seen, todo).is_ok() {
                        return Ok(());
                    }
                }
                ...
                err!(ValidationFailure(format!(
                    "encountered union {}whose bit pattern matches none of its {} fields",
                    place,
                    n,
                )))
            },
